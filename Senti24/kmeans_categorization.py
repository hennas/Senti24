...
import zipfs_law
import category_transitions

"""
K-Means categorization of the threads
"""

class Kmeans_categorization:
    def __init__(self, data, ids_by_year):
        self.data = data
        self.ids_by_year = ids_by_year
    ...
    def kmeans_main(self):
        ...
        categories = all_labels.replace([0,1,2,3,4,5],['Short Text','Question','Question/Descriptive','Negative text','Announcement','Rant'])
...
if __name__ == '__main__':
    ids_files = [r"finnpos\20{:02}_ids.txt".format(i+1) for i in range(8,17)]
    ids_by_year = []
    ...
    km_obj = Kmeans_categorization(data,ids_by_year)
    km_obj.kmeans_main()
    ...
    # Fit Zipf's law
    fig = zipfs_law.fit_zipfs_law(km_obj.all_data.kmeans_cat)
    fig.savefig('zipf_kmeans.png', format="png")
    
    # Calculate category transitions
    t_counts = category_transitions.calculate_category_transitions(km_obj.all_data.kmeans_cat)
    category_transitions.cross_table(km_obj.all_data.kmeans_cat.unique(), t_counts)
