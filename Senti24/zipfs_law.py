...
def get_category_ranks(val_dictionary):
    """
...
def fit_zipfs_law(cats):
    ...
    """
    freqs = get_category_freqs(cats)
    ranks = get_category_ranks(freqs)
    ...
    params, cov = curve_fit(power_law, [v[2] for v in freqs_ranks],[v[1] for v in freqs_ranks])
    #print(params)
    ...
    ax2.plot(ranks_sorted, power_law(np.array(ranks_sorted), *params), 'r--', linewidth=2, label="Power Law Fit")
    ax2.legend(loc="best")
...
if __name__ == '__main__':
    ...
    fig = fit_zipfs_law(data.simple_heuristic_cat)
